...
def list_fiis(user_id):
    try:
        ...
        favorites = {fav.fii_ticker for fav in FavoriteFii.query.filter_by(user_id=user_id).all()}
...
def update_all_fiis():
    try:
        ...
        new_fiis = [
            Fii(**fii_data)
            for fii_data in cached_fiis if fii_data['ticker'] not in existing_tickers
        ]
