...
def new_favorite_fii(favorite_data):
    ...
    existing_favorite = FavoriteFii.query.filter_by(user_id=user_id, fii_ticker=fii_ticker).first()
...
def add_favorite_fii(user_id, ticker):
    existing_favorite = FavoriteFii.query.filter_by(user_id=user_id, fii_ticker=ticker).first()
    if existing_favorite:
        return jsonify({'message': 'This FII is already favorited by this user'}), 400
